...
class NavigationDirtyUpstreamAuditTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.receipt = self.workspace / "private" / "navigation.json"
        self.write_receipt()
    ...
    def receipt_data(
        ...
        *,
        ...
        return {
            ...
            "repository_relpath": "third_party/service-navigation-rbnx",
    ...
    def test_matching_private_workspace_receipt_is_accepted(self) -> None:
        ...
        self.assertIn(
            "separately audited Navigation dirty patch accepted", result.stdout
        )
    ...
    def test_untracked_or_unexpected_tracked_paths_are_rejected(self) -> None:
        ...
        self.write_receipt()
        self.assert_rejected("not exactly the audited Navigation files")
    ...
    def test_receipt_must_be_private_regular_owned_file_inside_workspace(self) -> None:
        ...
        symlink = self.workspace / "private" / "navigation-link.json"
    ...
    def test_expiry_metadata_does_not_replace_exact_diff_validation(self) -> None:
        ...
        self.write_receipt(
            self.receipt_data(issued=issued, expires=issued + timedelta(hours=1))
        )
        self.assertEqual(self.verify().returncode, 0)

        issued = datetime.now(timezone.utc).replace(microsecond=0)
        self.write_receipt(
            self.receipt_data(issued=issued, expires=issued + timedelta(hours=5))
        )
        self.assertEqual(self.verify().returncode, 0)

    def test_shell_gate_exposes_only_label_specific_exceptions(self) -> None:
        ...
        self.assertEqual(source.count("ROBONIX_NAVIGATION_DIRTY_AUDIT_RECEIPT"), 2)
        ...
        self.assertNotIn("ALLOW_DIRTY", source)
