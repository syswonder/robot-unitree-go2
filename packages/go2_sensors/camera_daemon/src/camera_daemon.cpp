...
bool stream_camera(
  ...
  const int peer, std::uint64_t & sequence)
{
  using Clock = std::chrono::steady_clock;
  ...
  while (g_stop_requested == 0) {
    ...
    if (!go2_sensors::camera_ipc::jpeg_dimensions(jpeg.data(), jpeg.size(), width, height)) {
      std::cerr << "Rejected JPEG without valid encoded dimensions\n";
      next_sample = Clock::now() + period;
      sleep_until_or_stop(next_sample);
      continue;
    }
    if (sequence == std::numeric_limits<std::uint64_t>::max()) {
      std::cerr << "Camera sequence exhausted; stopping daemon\n";
      g_stop_requested = 1;
      return false;
    }
    ...
    if (header.capture_realtime_ns == 0U || header.capture_monotonic_ns == 0U) {
      std::cerr << "Clock read failed; dropping camera frame\n";
      continue;
    }
    ...
    if (!go2_sensors::camera_ipc::write_exact(
      ...
      !go2_sensors::camera_ipc::write_exact(
...
int run(const Options & options)
{
  ...
  while (g_stop_requested == 0) {
    ...
    stream_camera(video_client, options, peer, sequence);
