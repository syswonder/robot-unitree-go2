"""Validate and atomically consume one independent second-motion permit.

The second stage is deliberately not an expansion of the first 10 cm permit.
It has its own profile, topic, hard envelope and acknowledgement, and it binds
the already completed first-motion PASS receipt in addition to fresh DDS,
state and time evidence.
"""

from __future__ import annotations

import hashlib
import json
import math
import os
from pathlib import Path
import re
import stat
import time
from typing import Mapping, Sequence

from .runtime_config import (
    DEFAULT_EXTERNAL_ODOM_TOPIC,
    ODOM_SOURCE_EXTERNAL_VERIFIED,
    RuntimeConfig,
    SECOND_MOTION_COMMAND_TIMEOUT_S,
    SECOND_MOTION_COMMAND_TOPIC,
    SECOND_MOTION_CONTROL_RATE_HZ,
    SECOND_MOTION_MAX_ANGULAR_ACCEL_RPS2,
    SECOND_MOTION_MAX_DISTANCE_M,
    SECOND_MOTION_MAX_DURATION_S,
    SECOND_MOTION_MAX_LINEAR_ACCEL_MPS2,
    SECOND_MOTION_MAX_VX_MPS,
    SECOND_MOTION_MAX_VY_MPS,
    SECOND_MOTION_MAX_WZ_RPS,
    SECOND_MOTION_ODOM_TOPIC,
    SECOND_MOTION_PROFILE,
)


PERMIT_SCHEMA = "robonix-go2-second-motion-permit-v1"
PERMIT_ENV = "GO2_SECOND_MOTION_PERMIT_FILE"
SECOND_MOTION_ACK = "I_APPROVE_GO2_SECOND_20CM_MOTION"
FIRST_PASS_SCHEMA = "robonix-go2-first-motion-v1"
MAX_LIFETIME_NS = 5 * 60 * 1_000_000_000
MIN_REMAINING_NS = 15 * 1_000_000_000
MAX_JSON_BYTES = 64 * 1024
_HEX_RE = re.compile(r"^[0-9a-f]{64}$")
_ID_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_.-]{7,127}$")


class PermitError(ValueError):
    """The second-motion permit or its bound evidence is unsafe."""


def _unique_object(
    pairs: Sequence[tuple[str, object]],
) -> dict[str, object]:
    result: dict[str, object] = {}
    for key, value in pairs:
        if key in result:
            raise PermitError(f"duplicate JSON key is forbidden: {key}")
        result[key] = value
    return result


def _owned_regular_file(path: Path, *, private: bool) -> os.stat_result:
    try:
        info = os.lstat(path)
    except FileNotFoundError as exc:
        raise PermitError(f"required file does not exist: {path}") from exc
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode):
        raise PermitError(f"file must be a non-symlink regular file: {path}")
    if info.st_uid != os.geteuid():
        raise PermitError(f"file is not owned by the current UID: {path}")
    mode = stat.S_IMODE(info.st_mode)
    if private and mode != 0o600:
        raise PermitError(f"permit mode must be exactly 0600, got {mode:04o}")
    if not private and mode & 0o022:
        raise PermitError(f"evidence file is group/world writable: {path}")
    return info


def _read_json(path: Path, *, private: bool) -> dict[str, object]:
    expected = _owned_regular_file(path, private=private)
    descriptor = os.open(path, os.O_RDONLY | os.O_CLOEXEC | os.O_NOFOLLOW)
    try:
        opened = os.fstat(descriptor)
        if (
            opened.st_dev != expected.st_dev
            or opened.st_ino != expected.st_ino
            or opened.st_uid != os.geteuid()
            or not stat.S_ISREG(opened.st_mode)
        ):
            raise PermitError("file changed while it was being opened")
        raw = os.read(descriptor, MAX_JSON_BYTES + 1)
        if len(raw) > MAX_JSON_BYTES:
            raise PermitError("JSON file is unreasonably large")
    finally:
        os.close(descriptor)
    try:
        payload = json.loads(
            raw.decode("utf-8"), object_pairs_hook=_unique_object
        )
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise PermitError("file is not valid unique-key UTF-8 JSON") from exc
    if not isinstance(payload, dict):
        raise PermitError("JSON root must be an object")
    return payload


def sha256_file(path: Path) -> str:
    _owned_regular_file(path, private=False)
    digest = hashlib.sha256()
    with path.open("rb") as handle:
        for chunk in iter(lambda: handle.read(1024 * 1024), b""):
            digest.update(chunk)
    return digest.hexdigest()


def _exact_number(value: object, expected: float, label: str) -> None:
    if (
        isinstance(value, bool)
        or not isinstance(value, (int, float))
        or not math.isfinite(float(value))
        or float(value) != expected
    ):
        raise PermitError(f"{label} does not match the second-motion envelope")


def validate_first_motion_pass(path: Path) -> str:
    """Return the PASS receipt hash after validating its phase-transition facts."""

    payload = _read_json(path, private=False)
    if payload.get("schema") != FIRST_PASS_SCHEMA:
        raise PermitError("first-motion evidence schema is invalid")
    exact = {
        "status": "PASS",
        "command_topic": "/go2/commissioning/cmd_vel",
        "command_ownership": "PASS",
        "required_odom_source": "external_verified",
    }
    for key, expected in exact.items():
        if payload.get(key) != expected:
            raise PermitError(f"first-motion PASS evidence has invalid {key}")
    for key in (
        "commissioning_motion_active_observed",
        "disarm_success",
        "post_stop_success",
    ):
        if payload.get(key) is not True:
            raise PermitError(f"first-motion PASS evidence lacks {key}")
    _exact_number(payload.get("distance_limit_m"), 0.10, "first distance")
    _exact_number(payload.get("duration_limit_s"), 2.0, "first duration")
    forward = payload.get("measured_odom_forward_m")
    if (
        isinstance(forward, bool)
        or not isinstance(forward, (int, float))
        or not math.isfinite(float(forward))
        or float(forward) < 0.02
    ):
        raise PermitError("first-motion PASS has insufficient measured motion")
    post_stop = payload.get("post_stop")
    if (
        not isinstance(post_stop, dict)
        or post_stop.get("daemon_armed") is not False
        or post_stop.get("commissioning_motion_active") is not False
        or post_stop.get("guard_state") != "DISARMED"
    ):
        raise PermitError("first-motion PASS lacks measured stopped state")
    return sha256_file(path)


def validate_permit(
    payload: Mapping[str, object],
    runtime: RuntimeConfig,
    package_root: Path,
    *,
    now_unix_ns: int | None = None,
) -> str:
    """Return the permit id after validating every immutable claim."""

    now_ns = time.time_ns() if now_unix_ns is None else now_unix_ns
    if payload.get("schema") != PERMIT_SCHEMA:
        raise PermitError("unexpected second-motion permit schema")
    permit_id = payload.get("permit_id")
    session_id = payload.get("session_id")
    if not isinstance(permit_id, str) or _ID_RE.fullmatch(permit_id) is None:
        raise PermitError("permit_id is malformed")
    if not isinstance(session_id, str) or _ID_RE.fullmatch(session_id) is None:
        raise PermitError("session_id is malformed")
    if payload.get("one_time") is not True:
        raise PermitError("permit must explicitly be one_time")
    issued_ns = payload.get("issued_unix_ns")
    expires_ns = payload.get("expires_unix_ns")
    if (
        isinstance(issued_ns, bool)
        or isinstance(expires_ns, bool)
        or not isinstance(issued_ns, int)
        or not isinstance(expires_ns, int)
        or issued_ns <= 0
        or expires_ns <= issued_ns
        or expires_ns - issued_ns > MAX_LIFETIME_NS
    ):
        raise PermitError("permit lifetime is invalid or exceeds five minutes")
    if now_ns < issued_ns or expires_ns - now_ns < MIN_REMAINING_NS:
        raise PermitError("permit is not current or has less than 15 seconds left")

    exact_text = {
        "profile": SECOND_MOTION_PROFILE,
        "operator_ack": SECOND_MOTION_ACK,
        "network_interface": runtime.network_interface,
        "state_topic": runtime.state_topic,
        "command_topic": SECOND_MOTION_COMMAND_TOPIC,
        "odom_topic": SECOND_MOTION_ODOM_TOPIC,
        "external_odom_topic": DEFAULT_EXTERNAL_ODOM_TOPIC,
        "arm_service": runtime.arm_service,
    }
    for name, expected in exact_text.items():
        if payload.get(name) != expected:
            raise PermitError(f"permit {name} does not match this runtime")
    if runtime.motion_profile != SECOND_MOTION_PROFILE:
        raise PermitError("runtime is not the second-motion profile")
    if runtime.odom_source != ODOM_SOURCE_EXTERNAL_VERIFIED:
        raise PermitError("second motion requires external_verified odometry")
    if payload.get("allowed_modes") != list(runtime.allowed_modes):
        raise PermitError("permit allowed_modes does not match this runtime")
    if payload.get("allowed_state_markers") != list(
        runtime.allowed_state_markers
    ):
        raise PermitError(
            "permit allowed_state_markers does not match this runtime"
        )
    _exact_number(
        payload.get("max_linear_x_mps"),
        SECOND_MOTION_MAX_VX_MPS,
        "max_linear_x_mps",
    )
    _exact_number(
        payload.get("max_linear_y_mps"),
        SECOND_MOTION_MAX_VY_MPS,
        "max_linear_y_mps",
    )
    _exact_number(
        payload.get("max_angular_z_rps"),
        SECOND_MOTION_MAX_WZ_RPS,
        "max_angular_z_rps",
    )
    _exact_number(
        payload.get("max_linear_accel_mps2"),
        SECOND_MOTION_MAX_LINEAR_ACCEL_MPS2,
        "max_linear_accel_mps2",
    )
    _exact_number(
        payload.get("max_angular_accel_rps2"),
        SECOND_MOTION_MAX_ANGULAR_ACCEL_RPS2,
        "max_angular_accel_rps2",
    )
    _exact_number(
        payload.get("max_duration_s"),
        SECOND_MOTION_MAX_DURATION_S,
        "max_duration_s",
    )
    _exact_number(
        payload.get("max_distance_m"),
        SECOND_MOTION_MAX_DISTANCE_M,
        "max_distance_m",
    )
    _exact_number(
        payload.get("command_timeout_s"),
        SECOND_MOTION_COMMAND_TIMEOUT_S,
        "command_timeout_s",
    )
    _exact_number(
        payload.get("control_rate_hz"),
        SECOND_MOTION_CONTROL_RATE_HZ,
        "control_rate_hz",
    )

    evidence = payload.get("evidence")
    names = {"dds_identity", "state", "time", "first_motion_pass"}
    if not isinstance(evidence, dict) or set(evidence) != names:
        raise PermitError(
            "permit must bind identity, state, time, and first-motion PASS"
        )
    resolved_root = package_root.resolve()
    resolved_paths: dict[str, Path] = {}
    for name in sorted(names):
        claim = evidence[name]
        if not isinstance(claim, dict) or set(claim) != {"path", "sha256"}:
            raise PermitError(f"{name} evidence claim is malformed")
        path_text = claim.get("path")
        expected_hash = claim.get("sha256")
        if not isinstance(path_text, str) or not Path(path_text).is_absolute():
            raise PermitError(f"{name} evidence path must be absolute")
        if (
            not isinstance(expected_hash, str)
            or _HEX_RE.fullmatch(expected_hash) is None
        ):
            raise PermitError(f"{name} evidence hash is malformed")
        evidence_path = Path(path_text).resolve()
        try:
            evidence_path.relative_to(resolved_root)
        except ValueError as exc:
            raise PermitError(f"{name} evidence is outside package root") from exc
        if sha256_file(evidence_path) != expected_hash:
            raise PermitError(f"{name} evidence hash changed")
        resolved_paths[name] = evidence_path
    if (
        validate_first_motion_pass(resolved_paths["first_motion_pass"])
        != evidence["first_motion_pass"]["sha256"]
    ):
        raise PermitError("first-motion PASS validation hash mismatch")
    return permit_id


def consume_second_motion_permit(
    runtime: RuntimeConfig,
    environ: Mapping[str, str],
    package_root: Path,
) -> Path:
    """Validate and rename the permit so it cannot authorize a second arm."""

    if not runtime.allow_motion or runtime.motion_profile != SECOND_MOTION_PROFILE:
        raise PermitError("permit consumption requires second-motion runtime")
    path_text = environ.get(PERMIT_ENV, "").strip()
    if not path_text:
        raise PermitError(f"second motion requires {PERMIT_ENV}")
    permit_path = Path(path_text)
    if not permit_path.is_absolute():
        raise PermitError("second-motion permit path must be absolute")
    parent = os.lstat(permit_path.parent)
    if (
        not stat.S_ISDIR(parent.st_mode)
        or stat.S_ISLNK(parent.st_mode)
        or parent.st_uid != os.geteuid()
        or stat.S_IMODE(parent.st_mode) != 0o700
    ):
        raise PermitError("permit parent must be an owned 0700 directory")
    payload = _read_json(permit_path, private=True)
    permit_id = validate_permit(payload, runtime, package_root)
    consumed = permit_path.with_name(f"{permit_path.name}.consumed-{permit_id}")
    if consumed.exists() or consumed.is_symlink():
        raise PermitError("permit was already consumed")
    try:
        os.rename(permit_path, consumed)
    except FileNotFoundError as exc:
        raise PermitError("permit was consumed concurrently") from exc
    return consumed
