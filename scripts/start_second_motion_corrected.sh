...
# not a navigation entry point. It can run only the fixed 0.30 m/s, 1.2 s,
# 0.20 m forward probe on /go2/second_motion/cmd_vel.
...
MOTION_ACK=I_APPROVE_GO2_SECOND_20CM_MOTION
WIRELESS_ACK=I_APPROVE_GO2_WIRELESS_PRIVATE_LAN_SECOND_20CM
...
[[ "${GO2_SECOND_MOTION_ACK:-}" == "$MOTION_ACK" ]] || {
  echo "GO2_SECOND_MOTION_ACK must exactly equal $MOTION_ACK" >&2
  exit 2
}
...
case "$TRANSPORT" in
  ...
  wireless-private)
    [[ "${GO2_SECOND_MOTION_WIRELESS_ACK:-}" == "$WIRELESS_ACK" ]] || {
      echo "GO2_SECOND_MOTION_WIRELESS_ACK must exactly equal $WIRELESS_ACK" >&2
      exit 2
    }
    [[ "${GO2_NETWORK_INTERFACE:-}" == "$WIRELESS_INTERFACE" ]] || {
      echo "wireless second motion must use $WIRELESS_INTERFACE" >&2
      exit 2
    }
    ;;
  *)
    echo "GO2_SECOND_MOTION_TRANSPORT must be wired or wireless-private" >&2
    exit 2
    ;;
esac

for variable in GO2_NETWORK_INTERFACE GO2_SECOND_MOTION_PERMIT_FILE \
  GO2_SECOND_MOTION_STATE_EVIDENCE GO2_SECOND_MOTION_DDS_IDENTITY_EVIDENCE \
  GO2_TIMESTAMP_APPROVAL_FILE GO2_FIRST_MOTION_PASS_EVIDENCE; do
  value="${!variable:-}"
  [[ -n "$value" ]] || {
...
for variable in GO2_SECOND_MOTION_PERMIT_FILE \
  GO2_SECOND_MOTION_STATE_EVIDENCE GO2_SECOND_MOTION_DDS_IDENTITY_EVIDENCE \
  GO2_TIMESTAMP_APPROVAL_FILE GO2_FIRST_MOTION_PASS_EVIDENCE; do
  value="${!variable}"
...
second_motion_validate_wireless_topology() {
...
second_motion_validate_wireless_topology || exit 4
...
read -r observed_mode observed_marker observed_gait < <(
  "$PYTHON" "$ROOT/scripts/validate_first_motion_state_evidence.py" \
    "$GO2_SECOND_MOTION_STATE_EVIDENCE"
) || {
  echo "stationary second-motion state marker evidence is required" >&2
  exit 5
}
for value in "$observed_mode" "$observed_marker" "$observed_gait"; do
  [[ "$value" =~ ^[0-9]+$ ]] || {
    echo "validated second-motion state contains a malformed integer" >&2
    exit 5
  }
done
...
"$PYTHON" "$ROOT/scripts/validate_second_motion_permit_for_launch.py" \
  --permit "$GO2_SECOND_MOTION_PERMIT_FILE" \
  --package-root "$ROOT" \
  --network-interface "$GO2_NETWORK_INTERFACE" \
  --allowed-mode "$observed_mode" \
  --allowed-state-marker "$observed_marker" \
  --dds-evidence "$GO2_SECOND_MOTION_DDS_IDENTITY_EVIDENCE" \
  --state-evidence "$GO2_SECOND_MOTION_STATE_EVIDENCE" \
  --time-evidence "$GO2_TIMESTAMP_APPROVAL_FILE" \
  --first-motion-pass-evidence "$GO2_FIRST_MOTION_PASS_EVIDENCE" >/dev/null
...
export GO2_SECOND_MOTION_ACK="$MOTION_ACK"
...
second_motion_require_no_adapter() {
...
second_motion_require_no_adapter || exit 5
...
RUN_DIR="$(mktemp -d "$ROOT/rbnx-build/run/second-motion-corrected.XXXXXX")"
chmod 700 "$RUN_DIR"
...
"$PYTHON" "$ROOT/deploy/time-sync/render_workstation_second_motion_manifest.py" \
...
second_motion_require_no_adapter || exit 6
...
echo "================================================================"
echo " PRIVATE SECOND-MOTION GRAPH READY"
echo " max forward: 0.30 m/s"
echo " soft stop: 1.2 s / 0.20 m; hard envelope: 1.5 s / 0.30 m"
...
while true; do
  ...
  for pid in "$IDENTITY_PID" "$STAMP_PID" "$RELAY_PID" "$BOOT_PID"; do
    kill -0 "$pid" 2>/dev/null || {
      echo "a required second-motion process exited before adapter readiness" >&2
      exit 7
    }
  done
...
for pid in "$IDENTITY_PID" "$STAMP_PID" "$RELAY_PID" "$BOOT_PID"; do
  kill -0 "$pid" 2>/dev/null || {
    echo "a required second-motion process stopped before probe launch" >&2
    exit 7
  }
done
second_motion_validate_wireless_topology || {
  echo "wireless topology changed before the second-motion probe" >&2
  exit 7
}
