...
# Read-only graph gate for the independent second-motion path. This script
# never publishes, calls a service/action, or changes networking.

readonly topic=/go2/second_motion/cmd_vel
readonly canonical_topic=/cmd_vel
readonly expected_type=geometry_msgs/msg/Twist
readonly expected_publisher=go2_second_motion_probe
readonly expected_subscriber=go2_chassis_adapter
readonly discovery_timeout_s="${GO2_SECOND_MOTION_OWNERSHIP_TIMEOUT_S:-15}"
readonly stability_samples="${GO2_SECOND_MOTION_OWNERSHIP_STABILITY_SAMPLES:-3}"
readonly query_timeout_s="${GO2_SECOND_MOTION_OWNERSHIP_QUERY_TIMEOUT_S:-3}"
...
if (( discovery_timeout_s > 30 || stability_samples > 10 \
  || query_timeout_s > 10 )); then
  echo "second-motion ownership timing exceeds its audited maximum" >&2
  exit 2
fi
...
query_graph() {
  ...
  if [[ "$status" -ne 0 ]]; then
    ...
    echo "could not query dedicated second-motion topic: $output" >&2
    return 4
  fi
  ...
  publisher_count="$(
    sed -n \
      's/^Publisher count:[[:space:]]*\([0-9][0-9]*\)[[:space:]]*$/\1/p' \
      <<<"$output"
  )"
  subscription_count="$(
    sed -n \
      's/^Subscription count:[[:space:]]*\([0-9][0-9]*\)[[:space:]]*$/\1/p' \
      <<<"$output"
  )"
  if [[ "$type" != "$expected_type" || ! "$publisher_count" =~ ^[0-9]+$ \
      || ! "$subscription_count" =~ ^[0-9]+$ ]]; then
    echo "second-motion topic type/count output is ambiguous" >&2
    return 4
  fi
  if (( publisher_count != 1 || subscription_count != 1 )); then
    echo "second-motion ownership requires exactly one publisher and one subscription; found publishers=${publisher_count} subscriptions=${subscription_count}" >&2
    return 5
  fi
  ...
  publisher_namespaces="$(
    sed -n \
      's/^[[:space:]]*Node namespace:[[:space:]]*//p' \
      <<<"$publisher_section"
  )"
  ...
  if [[ "$publisher_names" != "$expected_publisher" \
    ...
    echo "second-motion endpoint identity/QoS does not match its dedicated path" >&2
    return 5
  fi
...
echo "================================================================"
echo " READ-ONLY SECOND-MOTION COMMAND OWNERSHIP CHECK"
echo " topic: $topic"
...
echo "dedicated second-motion endpoints did not become stable before deadline" >&2
exit 6
