#!/usr/bin/env python3
"""Verify an exact receipt for the audited dense-scan Mapping patch."""
...
def verify(workspace: Path, repo: Path, receipt: Path) -> None:
    ...
    try:
        ...
        raise AuditError("audited Mapping checkout must be inside the workspace") from error
    top_level = Path(_git(repo, "rev-parse", "--show-toplevel").decode().strip()).resolve()
    if top_level != repo:
        raise AuditError("audited Mapping path is not the git worktree root")

    raw_receipt = _read_private_receipt(receipt, workspace)
    data = _strict_json(raw_receipt)
    if data["schema"] != SCHEMA or data["repository"] != "mapping":
        raise AuditError("receipt is not an audited Mapping receipt")
    if data["repository_relpath"] != repository_relpath:
        raise AuditError("receipt Mapping checkout path does not match")
    if data["allowed_paths"] != list(ALLOWED_PATHS):
        raise AuditError("receipt allowed_paths is not the narrow audited Mapping set")
    ...
    if not re.fullmatch(r"[0-9a-f]{40}|[0-9a-f]{64}", head):
        raise AuditError("Mapping HEAD is not a canonical object id")
    if data["head"] != head:
        raise AuditError("audit receipt HEAD does not match Mapping HEAD")

    untracked = _git(repo, "ls-files", "--others", "--exclude-standard", "-z")
    if untracked:
        raise AuditError("audited Mapping checkout contains untracked files")
    ...
    if changed_paths != ALLOWED_PATHS:
        raise AuditError(
            "Mapping tracked changes are not exactly the audited Mapping files"
        )
    for relative in ALLOWED_PATHS:
        ...
        try:
            ...
            raise AuditError(f"cannot inspect audited Mapping file: {relative}") from error
        if not stat.S_ISREG(target_metadata.st_mode) or stat.S_ISLNK(target_metadata.st_mode):
            raise AuditError(f"audited Mapping path is not a regular file: {relative}")
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print("[upstream-compat] separately audited Mapping dirty patch accepted")
    return 0
