#!/usr/bin/env bash
# Read-only NX-side network identity check.  It never changes an interface.
...
if [[ "$(uname -m)" != "${expected_arch}" ]]; then
  echo "jetson-time-sync requires ARM64 (${expected_arch})" >&2
  exit 10
fi
...
if [[ "${#addresses[@]}" -ne 1 \
  || "${addresses[0]:-}" != "${expected_cidr}" ]]; then
